...
class LikedVolumeControllerTests {
	...
	@Test
	void retrieveAllLikedVolumes() throws Exception {
		// Call endpoint
	...
	@Test
	void userCanLikeVolume() throws Exception {
		// Get existing test data
		final Volume testVolume = volumeRepository.findAll().get(0);

		// Create a test user
		User user = new User("testuser_like_vol", "password123");
		final User savedUser = userRepository.save(user);

		// Record count before
		int countBefore = likedVolumeRepository.findAll().size();
		...
		int countAfter = likedVolumeRepository.findAll().size();
